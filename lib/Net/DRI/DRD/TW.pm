## Domain Registry Interface, .TW policies
##
...
Net::DRI::DRD::TW - .TW policies for Net::DRI
...
sub verify_name_domain
{
 ...
 ($domain, $op) = ($ndr, $domain) unless (defined($ndr) && $ndr &&
	(ref($ndr) eq 'Net::DRI::Registry'));

 my $r = $self->SUPER::check_name($domain, 1);
 $r = $self->SUPER::check_name($domain, 2) if ($r == 2);
 return $r if ($r);
