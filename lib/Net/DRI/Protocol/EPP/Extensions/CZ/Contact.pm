...
sub build_command
{
	...
	my @c = map { UNIVERSAL::isa($_, 'Net::DRI::Data::Contact') ?
		$_->srid() : $_ } @contact;
	...
	foreach my $n (@c)
	{
		Net::DRI::Exception->die(1, 'protocol/EPP', 2,
			'Contact id needed') unless (defined($n) && $n);
	...
	if (($tcommand =~ m/^(?:info|transfer)$/) && ref($contact[0]) &&
		UNIVERSAL::isa($contact[0], 'Net::DRI::Data::Contact'))
	{
...
sub create 
{
	...
	Net::DRI::Exception->die(1, 'protocol/EPP', 10, 'Invalid contact ' .
		$contact) unless (UNIVERSAL::isa($contact,
...
sub update 
{
	...
	Net::DRI::Exception::usererr_invalid_parameters($todo .
		...
		unless ($todo && ref($todo) &&
			$todo->isa('Net::DRI::Data::Changes'));
