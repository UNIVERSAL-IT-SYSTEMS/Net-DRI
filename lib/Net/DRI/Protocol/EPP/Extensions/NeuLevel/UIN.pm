...
Net::DRI::Protocol::EPP::Extensions::NeuLevel::UIN - EPP Extension for
	.travel UIN for Net::DRI
...
sub add_uin
{
 ...
 return unless (ref($rd) eq 'HASH' && exists($rd->{uin}) && defined($rd->{uin}));
...
sub renew
{
 ...
 if (ref($rd) eq 'HASH' && exists($rd->{uin}) && defined($rd->{uin}))
 {
  $info{UIN} = $rd->{uin};
 }
