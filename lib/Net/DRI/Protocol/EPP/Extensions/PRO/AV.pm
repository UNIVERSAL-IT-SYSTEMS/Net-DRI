...
sub build_command
{
	...
	$tcommand = (ref($command) ? $command->[0] : $command);
	@ns = @{$msg->ns->{av}};
	$msg->command([$command, 'av:' . $tcommand,
		sprintf('xmlns:av="%s" xsi:schemaLocation="%s %s"',
			$ns[0], $ns[0], $ns[1])]);
...
sub check_parse
{
	...
	$chkdata = $mes->get_content('chkData', $ns);
	return unless $chkdata;

	$cd = $chkdata->getFirstChild();
	...
	while (defined($cd) && $cd)
	{
		...
		unless ($cd->nodeType() == 1) ## only for element nodes
		{
...
sub info_parse
{
	...
	$infdata = $mes->get_content('infData', $ns);
...
sub create
{
	...
	push(@d, ['av:checkType', $rd->{type}]) if (verify_rd($rd, 'type'));
	push(@d, ['av:host', $rd->{host}]) if (verify_rd($rd, 'host'));
	push(@d, ['av:contact', ['av:contactId', $rd->{contact}->srid()]])
		if (verify_rd($rd, 'contact') &&
		    UNIVERSAL::isa($rd->{contact}, 'Net::DRI::Data::Contact'));
...
sub create_parse
{
	...
	$credata = $mes->get_content('creData', $ns);
	return unless $credata;

	$cd = $credata->getFirstChild();
	$pd = DateTime::Format::ISO8601->new();
