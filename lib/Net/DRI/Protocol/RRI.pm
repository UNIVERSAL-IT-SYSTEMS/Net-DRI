...
# use Net::DRI::Protocol::RRI::Status;
...
sub new
{
 ...
 $self->factories({ 
                   ...
                   status  => sub { return Net::DRI::Protocol::RRI::Status->new(); },
...
sub parse_status
{
 my $node=shift;
 my %tmp;
 $tmp{name}=$node->firstChild()->getData() if ($node->firstChild());
 return \%tmp;
}
