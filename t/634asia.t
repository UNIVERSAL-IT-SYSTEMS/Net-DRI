...
eval {
	...
	$dri->target('ASIA')->new_current_profile('p1',
		'Net::DRI::Transport::Dummy',
		[{f_send => \&mysend, f_recv => \&myrecv}],
			'Net::DRI::Protocol::EPP::Extensions::ASIA', ['1.0', [':full']]);
};
if ($@)
{
...
eval {
	$rc = $dri->domain_create_only('epptest.asia', {
		contact =>	$cs,
		ns =>		$ns,
		url =>		'http://www.justfuckinggoogleit.com/',
		auth =>		{ pw => 'bleblubleu' }
	});
};
if ($@)
...
is($dri->get_info('url', 'domain', 'epptest23.asia'),
	'http://www.justfuckinggoogleit.com/', 'domain contact url');
